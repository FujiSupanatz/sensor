...
void saveCountToEEPROM(int count) {
  // Calculate the EEPROM addresses to store the integer count
  int address = 0; // Starting address in EEPROM
  EEPROM.put(address, count); // Store count at the calculated address
}

void readCountFromEEPROM() {
  int address = 0; // Starting address in EEPROM to read
  int storedCount;
  EEPROM.get(address, storedCount); // Retrieve count from EEPROM

  // Print the stored count to Serial monitor
